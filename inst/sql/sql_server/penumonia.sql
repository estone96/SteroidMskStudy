...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (257315,4049965,252655,260754,443410,440431,255848,254066,260430,258180,253790,252351,261324,4050872,259852,261326)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (257315,4049965,252655,260754,443410,440431,255848,254066,260430,258180,253790,252351,261324,4050872,259852,261326)
...
select event_id, person_id, 
  case when DATEADD(day,21,start_date) > start_date then DATEADD(day,21,start_date) else start_date end as end_date
INTO #strategy_ends
